...
for model_size, batch_size in zip(model_sizes, batch_sizes):
    for csv_path in list(training_df_folder.glob('*_REG.csv')):
        name = csv_path.stem
        print(name + '_ResNet' + str(model_size))
        os.system(f'qsub -l gpu=1 -l h_vmem=64G -pe threaded 8 submit.sh {name}_Regression {csv_path} {model_size} {batch_size}')
